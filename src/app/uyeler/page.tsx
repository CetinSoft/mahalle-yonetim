...
export default async function UyelerPage({
    ...
    searchParams: { yargitay?: string; cinsiyet?: string; gorevi?: string; mahalle?: string; ilce?: string; arama?: string; sayfa?: string; siralama?: string; siraDir?: string }
}) {
    ...
    if (isSuperAdmin) {
        ...
        // İlçe admini - yetkili olduğu ilçelerin mahallelerini al
        for (const ilce of userIlces) {
            const districtMahalles = await getDistrictMahalles(ilce)
            userMahalles.push(...districtMahalles)
        }
        userMahalles = [...new Set(userMahalles)] // Tekrarları kaldır
    } else if (tcNo) {
        // Normal kullanıcı - UserMahalle tablosundan yetkilerini al
    ...
    if (userMahalles.length === 0 && !isSuperAdmin && !isDistrictAdmin) {
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <a
                                    href={`/api/export/kisiler?${new URLSearchParams({
                                        ...
                                        ...(selectedMahalle && isSuperAdmin && { mahalle: selectedMahalle }),
                                        ...(arama && { arama }),
                                    }).toString()}`}
                        ...
                        <div className="flex flex-wrap gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                            ...
                            <div className="space-y-2 flex-1 min-w-[200px]">
                                ...
                                <form method="GET" className="flex gap-2">
                                    ...
                                    {selectedMahalle && isSuperAdmin && <input type="hidden" name="mahalle" value={selectedMahalle} />}
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {citizens.length === 0 ? (
                                        <tr>
                                            <td colSpan={isSuperAdmin && !selectedMahalle ? 8 : 7} className="px-6 py-8 text-center text-gray-500">
                                            ...
                                            return (
                                                <tr key={citizen.id} className="hover:bg-gray-50 transition-all group">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        {citizen.telefon ? (
                                                            <a
                                                                href={`tel:${formatPhoneNumber(citizen.telefon)}`}
                                                            ...
                                                            >
                                                                ...
                                                                {formatPhoneNumber(citizen.telefon)}
                                                            </a>
                                                        ) : (
